...
exports.parseResponse = function parseResponse(raw) {
    ...
    var certsFiltered = [];
    var certsTbsFiltered = [];
    if (basic.certs) {
        for (var i = 0; i < basic.certs.length; i++) {
            var cert = basic.certs[i];
            var tbs = certsTbs[i];

            var certOK = cert.tbsCertificate.extensions.some(function (ext) {
            ...
            if (certOK) {
                certsFiltered.push(cert);
                certsTbsFiltered.push(tbs);
            }
        }
    }
